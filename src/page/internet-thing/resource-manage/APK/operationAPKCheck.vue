  	  ...
  	  <el-col :span="24">
	  	  <el-menu :default-active="activeIndex" class="el-menu-demo" mode="horizontal" @select="handleSelect">
		      ...
		      <el-table-column
			    ...
			    <div>
			    	<el-form :model="detailForm" class="g-form-ipt" disabled="disabled">
			    	  <el-form-item label="大小:" :label-width="formLabelWidth">
					      <el-input v-model="detailForm.pageSize" auto-complete="off" class="g-item-ipt fl"></el-input>
					    </el-form-item>
					    <el-form-item label="路径:" :label-width="formLabelWidth">
					      <el-input v-model="detailForm.pagePath" auto-complete="off" class="g-item-ipt fl"></el-input>
					    </el-form-item>
					    <el-form-item label="版本:" :label-width="formLabelWidth">
					      <el-input v-model="detailForm.fileVersion" auto-complete="off" class="g-item-ipt fl"></el-input>
					    </el-form-item>
					    <el-form-item label="名称:" :label-width="formLabelWidth">
					      <el-input v-model="detailForm.fileName" auto-complete="off" class="g-item-ipt fl"></el-input>
					    </el-form-item>
					    <el-form-item label="上传用户:" :label-width="formLabelWidth">
					      <el-input v-model="detailForm.userName" auto-complete="off" class="g-item-ipt fl"></el-input>
					    </el-form-item>
			    	</el-form>
			    </div>
				</el-dialog>
			    ...
			    <!-- <el-form class="g-form-ipt">
			    	<el-input type="textarea" v-model="checkCauseText"></el-input>
			    	<el-button type="info" class="g-cancel fl" size="mini" @click="closePassVisible">
  	          取消
	    	    </el-button>
			    	<el-button type="primary" class="g-cancel fl" size="mini" @click="updateTaskInfo(status[1])">
  	          确定
	    	    </el-button>
			    </el-form> -->
				</el-dialog>
...
import { queryApkInfo, queryTaskInfo, queryFileTask, updateTaskInfo } from '@/service/iot-frame'
...
export default {
	...
	methods: {
		...
		async queryFileTask () {
			...
			let resData = await queryFileTask('?pageSize=' + self.paginationObj.pageSize + '&currentPage=' + self.paginationObj.currentPage, paramData)
		...
		async queryApkInfo (id) {
			let self = this
			let resData = await queryApkInfo({ "id": id })
			if (resData.status !== 200) {
        self.$message.error("服务器错误")
        return
      }
			if (resData.status === 200 && resData.data.status == '200') {
				...
				obj.pageSize = changePageSize(obj.pageSize, 1);
				self.detailForm = obj
			} else {
				self.$message.error(resData.data.message)
			}
